            ...
            if ($control instanceof Zend_Gdata_App_Extension_Control) {
                if (($control->getDraft() != null) &&
                    ...
                    if ($state instanceof Zend_Gdata_YouTube_Extension_State) {
                        $message = 'Статус загрузки: ' . $state->getName() . ' '
                            . $state->getText();
                    } else {
                        print $message;
                    }
...
function generateUrlInformation()
{
    if (!isset($_SESSION['operationsUrl']) || !isset($_SESSION['homeUrl'])) {
        $_SESSION['operationsUrl'] = 'http://'. $_SERVER['HTTP_HOST']
                                   . $_SERVER['PHP_SELF'];
        $path = explode('/', $_SERVER['PHP_SELF']);
        $path[count($path)-1] = 'session_details.php';
        $_SESSION['homeUrl'] = 'http://'. $_SERVER['HTTP_HOST']
                             . implode('/', $path);
    }
}
 ...
 * @return void
 */
function editVideoData($newVideoTitle, $newVideoDescription, $newVideoCategory, $newVideoTags, $videoId, $newID)
{
    $httpClient = getAuthSubHttpClient();
    $youTubeService = new Zend_Gdata_YouTube($httpClient);
    ...
    $edit_data=array();
    if( !empty($newID) ) {
        $edit_data['youTubeId'] = $newID;
    } 

    $edit_data['title'] = $newVideoTitle;
    $edit_data['description'] = $newVideoDescription;
    $edit_data['category'] = $newVideoCategory;
    $edit_data['tags'] = $newVideoTags; 

    global $db;
    $db->update("video", $edit_data, "youTubeId='".$videoId."'");
    ...
    if (!$videoEntryToUpdate instanceof Zend_Gdata_YouTube_VideoEntry) {
        print '<meta http-equiv="refresh" content="0; url=http://arrko.grammaticastudio.ru/acp/index.php?mid=video&mode=manage&info=while_edit_err&edit=success" />ERROR - Could not find a video while editing entry with id ' . $videoId
    ...
    try {
        ...
        print ' <meta http-equiv="refresh" content="0; url=http://arrko.grammaticastudio.ru/acp/index.php?mid=video&mode=manage&option=whileedit2&edit=success" />ERROR - Could not obtain video  while editing entry\'s edit link: '
            . $e->getMessage() . '<br />';
        return;
    }
    ...
    try {
        ...
        print 'ERROR ' . $httpException->getMessage()
            ...
            . '</textarea><br />'
            . '<a href="session_details.php">'
            . 'посмотреть детали последнего запроса</a><br />';
        return;
    } catch (Zend_Gdata_App_Exception $e) {
        print '<meta http-equiv="refresh" content="0; url=http://arrko.grammaticastudio.ru/acp/index.php?mid=video&mode=manage&edit=success&error_log=true" /> Упс! Кажется что-то пошло не так :( ERROR - Could not post video meta-data: ' . $e->getMessage();
        return;
    }
        print ' <meta http-equiv="refresh" content="0; url=http://arrko.grammaticastudio.ru/acp/index.php?mid=video&mode=manage&edit=success" />  <!-- Информация о видеозаписи сохранена<br /><a href="#" onclick="'
...
function createUploadForm($videoTitle, $videoDescription, $videoCategory, $videoTags, $nextUrl = null)
{
    ...
    //As documented, the title field has a maximum length of 60 characters when set/updated via the API https://groups.google.com/forum/#!topic/youtube-api-gdata/RzzD0MxFLL4
    //Place to redirect user after upload
    if (!$nextUrl) {
        $nextUrl = $_SESSION['homeUrl'];
        $nextUrl.= "&tags=$videoTags&category=$videoCategory&title=$videoTitle&description=$videoDescription";
    ...
    print <<< END
        ...
        method="post" enctype="multipart/form-data" id="startupload">
        <input name="file" type="file"/>
        <input name="token" type="hidden" value="${tokenValue}"/>
        <input value="Начать загрузку" type="submit" class="bottom-button" />
        </form>
END;
...
function deleteVideo($videoId)
{
    ...
    global $db;
    $db->query("DELETE FROM `video` WHERE youTubeId='".$videoId."'");
    ...
    if (!$videoEntryToDelete instanceof Zend_Gdata_YouTube_VideoEntry) {
        print '<meta http-equiv="refresh" content="0; url=http://arrko.grammaticastudio.ru/acp/index.php?mid=video&mode=manage&delete=success" /><!-- ERROR - Could not find a video entry with id --><!--' . $videoId . '<br />-->';
        return;
    }
    ...
    try {
        ...
        print '<meta http-equiv="refresh" content="0; url=http://arrko.grammaticastudio.ru/acp/index.php?mid=video&mode=manage&delete=success" /> <!-- ERROR -->' . $httpException->getMessage()
         . ' HTTP details<br /><textarea cols="100" rows="20">'
         . $httpException->getRawResponseBody()
         . '</textarea><br />'
         . '<a href="session_details.php">'
        ...
        print '<meta http-equiv="refresh" content="0; url=http://arrko.grammaticastudio.ru/acp/index.php?mid=video&mode=manage&delete=success" /><!--  ERROR - Could not delete video: -->'. $e->getMessage();
        return;
    }
    
    print '<meta http-equiv="refresh" content="0; url=http://arrko.grammaticastudio.ru/acp/index.php?mid=video&mode=manage&delete=success" /><!--Entry deleted succesfully.<br />' . $httpResponse->getBody()
        . '<br /><a href="#" onclick="'
        . 'ytVideoApp.presentFeed(\'search_owner\', 5, 0, \'none\');"'
        . '">(refresh your video listing)</a><br /> -->'
        . printCacheWarning();
...
function echoThumbnails($feed)
{
    foreach ($feed as $entry) {
        ...
        echo '<img id="' . $videoId . '" class="thumbnail" src="'
            . $firstThumbnail1111111 .'" width="130" height="97" onclick="'
...
function echoVideoList($feed, $authenticated = false)
{
    ...
    $table = '<div id="content" class="container"><table id="content_table" style="width: auto;
margin: auto;"><tr><td style="padding:50px 0px;">
    <h1>Видеоархив</h1>';
    $results = 0;
    ...
    foreach ($feed as $entry) {
        ...
        $slice="171";
        $words="20";
        if(mb_strlen($videoDescription, 'UTF-8') > $slice) {
            //$videoDescription=mb_substr($videoDescription, 0, $slice, 'UTF-8');
            $videoDescription=explode(' ', $videoDescription);
            foreach ($videoDescription as $key => $value) {
              if($key > $words) {
                unset($videoDescription[$key]);
              }  
            }
             $videoDescription=implode(' ', $videoDescription);
            $videoDescription.="...";
        }
...
/*
        $table .= '<tr id="video_' . $videoId . '">'
                . '<td width="130"><img onclick="ytVideoApp.presentVideo(\''
                . $videoId. '\')" src="' . $thumbnailUrl. '" /></td>'
                . '<td><b>'. stripslashes($videoTitle) . '</b>'
                . '<p class="videoDescription">'
                . stripslashes($videoDescription) . '</p>'
                . '<p class="videoCategory">Категория: ' . $videoCategory
                . '</p><p class="videoTags">tagged: '
                . htmlspecialchars(implode(', ', $videoTags)) . '</p>';
*/

        $table.='   <div class="block video">
        <iframe width="425" height="266" src="//www.youtube.com/embed/'.$videoId.'" frameborder="0" allowfullscreen></iframe>
        <div class="video_title">'. stripslashes($videoTitle) . '</div>
        <div class="video_text">'.stripslashes($videoDescription).'</div>
    </div>';

//$table.='</td></tr></table></div>';
          if ($authenticated) {
              $table .= '<div class="editpanel"><p class="edit">'
                     . '<input type="button" onclick="ytVideoApp.presentMetaDataEditForm(\''
                     . addslashes($videoTitle) . '\', \''
                     . addslashes($videoDescription) . '\', \''
                     . $videoCategory . '\', \''
                     . addslashes(implode(', ', $videoTags)) . '\', \''
                     . $videoId . '\');" value="Редактировать" class="bottom-button edit"> '
                     . '<input type="button" onclick="ytVideoApp.confirmDeletion(\''
                     . $videoId
                     . '\');" class="bottom-button" value="Удалить"></p><br clear="all"></div>';
          }
   ...
   // $table .= '</td></tr>';
    ...
    if ($results < 1) {
        echo '<br />Ничего не найдено<br /><br />';
    } else {
...
function echoVideoMetadata($entry)
{
    ...
    print <<<END
        ...
        <b>Автор:</b> <a href="${authorUrl}">${authorUsername}</a><br />
        <b>Теги:</b> ${tags}<br />
        <b>Продолжительность:</b> ${duration} seconds<br />
        <b>Количество просмотров:</b> ${viewCount}<br />
        <b>Рейтинг:</b> ${rating} (${numRaters} ratings)<br />
        <b>Flash:</b> <a href="${flashUrl}">${flashUrl}</a><br />
        <b>Страница просмотра:</b> <a href="${watchPage}">${watchPage}</a> <br />
END;
}
...
function createPlaylist($playlistTitle, $playlistDescription)
{
    ...
    print 'Playlist added succesfully.<br /><a href="#" onclick="'
        ...
        . '">(обновить список видеозаписей)</a><br />'
        . printCacheWarning();
}
