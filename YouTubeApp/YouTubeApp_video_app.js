...
ytVideoApp.sendRequest = function(filePath, params, resultDivName) {
  ...
  xmlhr.onreadystatechange = function() {
    ...
    if (xmlhr.readyState == 1) {
      ...
      console.log('Invalid response received - Status: ' + xmlhr.status);
    }
  }
  xmlhr.send(params);
}
...
ytVideoApp.prepareUploadForm = function() { 
  var  metaDataForm = ['<br clear="all"><form id="uploadForm" ',
    ...
    'return false;">',
    '<div class="input-title">Заголовок</div><input name="videoTitle" ',
    'type="text"  class="header"/><br /> ',
    '<div class="input-title">Описание<div/><textarea cols="50" ',
    'name="videoDescription" class="header"></textarea><br />',
    '<!-- Категория <select name="videoCategory">',
    ...
    '</select><br /> --> <input type="hidden" name="videoCategory" value="News">',
    '<div class="input-title">Теги ',
    '<em>(через запятую)</em>:</div>',
    '<input name="videoTags" type="text" size="50" value="video" class="header" /><br /><div class="input-title" id="existform">Ссылка на видео <small>(например, youtube.com/watch?v=lUDQnn-r7F4)</small></div><input type="text" id="addexistinput" class="header">',
    '<br><input type="submit" value="Добавить существующее видео" class="bottom-button" id="addexist"> <input type="submit" value="Перейти к выбору файла" class="bottom-button" id="startuploadd"> <br><input type="submit" value="Добавить" class="bottom-button"  id="startadd"><br><br>',
    '</form>'].join('');
...
ytVideoApp.presentMetaDataEditForm = function(oldVideoTitle, oldVideoDescription, oldVideoCategory, oldVideoTags, videoId) {
  ...
  var editMetaDataForm = ['<form id="editForm" ',
    ...
    '<input class="header" size="50" name="newVideoTitle" ',
    'type="text" value="',
    oldVideoTitle,
    '"/><br />',
    '<div class="input-title">Описание</div>',
    '<textarea name="newVideoDescription" class="header" style="height:100px;resize:none;">', 
    oldVideoDescription,
    '</textarea><br />',
    '<!-- <div class="input-title">Категория</div> <select ',
    ...
    '<option value="Nonprofit">Nonprofit &amp; Activism</option>',
    '<option value="Tech">Science &amp; Technology</option>',
    '</select><br /> --> <input type="hidden" name="newVideoCategory" value="News">',
    '<div class="input-title">Теги <small>(разделяются пробелом)</small></div>',
    '',
    '<input class="header" name="newVideoTags" type="text" size="50" ',
    'value="',
    oldVideoTags,
    '"/><br />',
    ...
    videoId,
    '" /><br />',
    '<input type="submit" value="Сохранить" class="bottom-button"> <input type="button" youtube="',videoId,'" value="Удалить" class="bottom-button" id="deletevideo"> <!-- &nbsp;<input type="submit" value="Отмена" id="cancel" class="bottom-button"> -->',
    '</form>'].join('');
 ...
 * @param {String} videoId The id of the video to be edited
 */
ytVideoApp.editMetaData = function(newVideoTitle, newVideoDescription, newVideoCategory, newVideoTags, newID, videoId) {
  var filePath = '/operations.php';
  ...
  window.location.href = "/acp/index.php?mid=video&mode=manage";
  //ytVideoApp.presentFeed('search_owner', 2000, 0, 'none');
...
ytVideoApp.confirmDeletion = function(videoId) {
  var answer =  confirm('Вы действительно хотите удалить это видео?');
  if (answer) {
